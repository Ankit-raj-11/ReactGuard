...
// ReactGuard: Autonomous on-chain DeFi guardian powered by Somnia Native Reactivity.
//
// Architecture:
...
contract ReactGuard is SomniaEventHandler {
    ...
    function createSubscription() external onlyOwner returns (uint256) {
        ...
        ISomniaReactivityPrecompile.SubscriptionData memory data =
            ISomniaReactivityPrecompile.SubscriptionData({
                ...
                handlerContractAddress:  address(this),     // ReactGuard handles it
    ...
    function _onEvent(
        ...
        require(emitter == oracle, "ReactGuard: Wrong oracle");
        require(
            eventTopics.length > 0 && eventTopics[0] == PRICE_DROP_TOPIC,
            "ReactGuard: Wrong event sig"
        );
