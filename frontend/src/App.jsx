...
export default function App() {
  ...
  const initContracts = useCallback(async (provider) => {
    try {
      ...
      pool.on('Paused', (by) => {
        setPoolPaused(true)
        addEvent('defend', `🛡️ Pool PAUSED by ReactGuard (${shortAddr(by)}). All borrows now reverting.`)
      })
      ...
      setLoading(false)
      addEvent('warning', `⚠️ Could not connect to contracts. Make sure VITE_ env vars are set after deploying.`)
    }
  }, [addEvent])
  ...
  useEffect(() => {
    if (!ORACLE_ADDR || !GUARD_ADDR || !POOL_ADDR) {
      setLoading(false)
      addEvent('warning', '⚠️ No contract addresses found. Deploy contracts and set VITE_ env vars.')
      return
    }
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      if (err.code === 4001) {
        addEvent('warning', '⚠️ Connection rejected — please approve in MetaMask.')
      } else if (err.code === -32002) {
        addEvent('warning', '🦊 MetaMask popup already open — click the MetaMask icon in your browser toolbar to approve.')
      } else {
        addEvent('warning', `⚠️ Wallet error: ${err.message ?? 'Unknown error'}`)
      }
    }
  }
  ...
  const simulateAttack = async () => {
    ...
    try {
      ...
      if (data.defended) {
        ...
        addEvent('defend', '🛡️ POOL PAUSED — attacker borrows REVERT. Zero bots used.')
      } else {
        addEvent('info', '⏳ Subscription active. Waiting for validator network latency...')
      }
      setTxStep('')
    } catch (err) {
      addEvent('warning', `⚠️ Attack failed: ${err.message?.slice(0, 100)}`)
      setTxStep('')
    } finally {
      setAttacking(false)
    }
  ...
  const resetDemo = async () => {
    ...
    try {
      ...
      addEvent('warning', `⚠️ Reset failed: ${err.message?.slice(0, 100)}`)
      setTxStep('')
    } finally {
      setResetting(false)
    }
  ...
  const statusLabel = poolPaused ? '⚠️ PROTOCOL PAUSED — UNDER ATTACK DEFENSE' : '✅ PROTOCOL ACTIVE — ALL SYSTEMS NOMINAL'
  const statusIcon  = poolPaused ? '🚨' : '🛡️'
      ...
      <header className="header">
        <div className="container header-inner">
          ...
          <div className="header-badges">
            <span className="badge badge-blue">Somnia Testnet</span>
            <span className="badge badge-purple">Reactivity v1</span>
            {!connected
              ? <button className="connect-btn" onClick={connectWallet}>🔗 Connect Wallet</button>
              : <div className="connected-pill">🟢 {shortAddr(wallet)}</div>
            }
          </div>
        </div>
      </header>
          ...
          <div className="grid-3">
            <div className="card">
              <div className="card-header">
                <div className="card-title"><span className="icon">📈</span> Oracle Price</div>
              </div>
            ...
            <div className="card">
              <div className="card-header">
                <div className="card-title"><span className="icon">⚡</span> Reactivity Engine</div>
            ...
            <div className="card">
              <div className="card-header">
                <div className="card-title"><span className="icon">🏦</span> Lending Pool</div>
              </div>
              <div className={`card-value ${poolPaused ? 'val-red' : 'val-green'}`}>
                {loading ? '…' : poolPaused ? 'PAUSED' : 'ACTIVE'}
              </div>
              <div className="card-sub">
                {poolPaused ? '⚠️ Borrows reverting — pool protected' : '✅ Deposits & borrows open'}
              </div>
          ...
          <div className="grid-2">
            ...
            <div className="feed-card">
              <div className="feed-header">
                <div className="feed-title">📡 Live Event Stream</div>
                <div className="feed-live"><div className="feed-live-dot" />LIVE</div>
              </div>
              <div className="feed-list">
                {events.length === 0
                  ? <div className="feed-empty"><div className="feed-empty-icon">📡</div>Waiting for on-chain events…</div>
                  : events.map(ev => (
            ...
            {/* Attack Controls + Latency */}
            <div style={{display:'flex',flexDirection:'column',gap:'20px'}}>

              {/* Latency card */}
              <div className="card">
                <div className="card-header">
                  <div className="card-title"><span className="icon">⏱</span> Defense Latency</div>
              ...
              <div className="sim-card">
                ...
                <h3>⚡ Demo Control — Oracle Manipulation</h3>
              ...
              <p className="simulator-help">
                ...
                {txStep && (
                  <div className="sim-progress">
                    <div className="sim-spinner" />{txStep}
                  </div>
                )}
